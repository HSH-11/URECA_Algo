...
//DFS
public class BOJ_파이프옮기기_DFS {

	static int N, cnt;
	static int[][] map;
	//delta
	//현재 방향에 따라 다음 이동 방향이 제한 <= 현재 방향에 따른 y,x의 변화량
	//현재=>다음
	static int[][][] delta = {
	...
	public static void main(String[] args) throws Exception{
		...
		dfs(1,2,1); //1,2좌표에서 가로인 상태
		
		System.out.println(cnt);
		

	}
	static void dfs(int y, int x, int d) { // d:현재 y,x좌표에 놓여진 방향
		// 현재 좌표가 N,N인지 확인
		if ( y == N && x == N) {
			cnt++;
			return;
		...
		//delta를 이용한 탐색을 이어나간다. for 문이 기저조건 역할
		for (int i = 0; i < 3; i++) { //i는 대각선,가로,세로
